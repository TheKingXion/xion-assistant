...
Estado: `v0.10.7` foundation. Incluye Command Registry autenticado, shortcuts privados, metricas, UI, migracion `0002`, guia Cloudflare cloud-first solo con GitHub/Dashboard, pasos para sacar `CLOUDFLARE_API_TOKEN`, pasos Google OAuth para dominio real y proteccion `keep_vars = true` para que deploys Worker no borren variables del dashboard. Deploy real requiere recursos y secrets Cloudflare.
