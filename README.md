...
Version `v0.11.1` is a foundation, not the complete final assistant. It adds authenticated Command Registry routing before AI, private shortcuts, usage metrics, confirmation-safe actions, a full web dashboard, Google login/register, Google Gemini text/TTS gateway support, GitHub/Dashboard-only Cloudflare deployment docs, Google OAuth dashboard steps for the real domain, and `keep_vars = true` so Worker deploys do not wipe dashboard variables. Desktop/mobile installers, STT implementation, R2 uploads, and production releases remain documented next phases.
