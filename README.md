...
Version `v0.9.0` is a foundation, not the complete final assistant. It implements the first executable slice plus D1-backed persistence, contact routing, a mock/configurable AI Gateway, tool registry, OAuth callback token exchange, Google Calendar list/create connector flow, Spotify playback controls behind explicit confirmation, and YouTube read endpoints. Desktop/mobile installers, real TTS/STT providers, R2 uploads, and production deployments remain documented next phases.
